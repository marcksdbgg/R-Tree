...
// Search
vector<vector<pair<int, int>>> RTree::Search(const int a_min[2], const int a_max[2])
{
    vector<vector<pair<int, int>>> result;
    Search(m_root, a_min, a_max, result);
    return result;
}

void RTree::Search(Node* a_node, const int a_min[2], const int a_max[2], vector<vector<pair<int, int>>>& result)
{
    if (a_node->IsLeaf())
    {
        for (int i = 0; i < a_node->m_count; ++i)
        {
            if (Overlap2((Rect*)a_min, (Rect*)&a_node->m_branch[i].m_rect))
            {
    ...
    else
    {
        for (int i = 0; i < a_node->m_count; ++i)
        {
            if (Overlap2((Rect*)a_min, &a_node->m_branch[i].m_rect))
            {
                Search(a_node->m_branch[i].m_child, a_min, a_max, result);
            }
        }
    }
}
...
// Get window size
void RTree::GetWindowSize(int& width, int& height)
{
    Rect mbr = NodeCover(m_root);
    int mbr_width = mbr.m_max[0] - mbr.m_min[0];
    int mbr_height = mbr.m_max[1] - mbr.m_min[1];
    float aspect_ratio = (float)mbr_width / (float)mbr_height;
    if (aspect_ratio > 1.0f)
    {
        width = 800;
        height = (int)(800.0f / aspect_ratio);
    }
    else
    {
        height = 800;
        width = (int)(800.0f * aspect_ratio);
    }
}

void RTree::DrawTree(Node* a_node)
{
    int width, height;
    ...
    if (a_node->IsInternalNode())
    {
        for (int i = 0; i < a_node->m_count; i++)
        {
            DrawTree(a_node->m_branch[i].m_child);
        }
    }
    else
    {
        for (int i = 0; i < a_node->m_count; i++)
        {
            Rect r = a_node->m_branch[i].m_rect;
            float x1 = (float)r.m_min[0] / (float)m_unitSize * (float)width;
            float y1 = (float)r.m_min[1] / (float)m_unitSize * (float)height;
            float x2 = (float)r.m_max[0] / (float)m_unitSize * (float)width;
            float y2 = (float)r.m_max[1] / (float)m_unitSize * (float)height;
